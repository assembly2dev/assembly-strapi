/**
 * API-based instructor import script
 * This script uses the Strapi API to create instructors
 * Usage: node scripts/import-instructors-api.js
 */
...
const API_TOKEN = process.env.STRAPI_API_TOKEN || ''; // Add your API token here
...
async function createInstructor(instructorData) {
  try {
    ...
    console.error(`Error creating instructor ${instructorData.name}:`, error.response?.data || error.message);
    return null;
  }
}

async function importInstructors() {
  console.log('Starting instructor import via API...');
  console.log(`Target Strapi URL: ${STRAPI_URL}`);
  
  if (!API_TOKEN) {
    console.log('Warning: No API token provided. Make sure your Strapi instance allows public access to instructor creation.');
  }
  ...
  for (const instructorData of instructorsData) {
    ...
    if (result) {
      console.log(`✅ Created: ${instructorData.name} (ID: ${result.data?.id})`);
      results.push(result);
    } else {
      console.log(`❌ Failed: ${instructorData.name}`);
    }
  ...
  console.log(`\nImport completed!`);
  console.log(`Successfully created: ${results.length}/${instructorsData.length} instructors`);
  
  if (results.length < instructorsData.length) {
    console.log('\nSome instructors failed to create. Check the errors above.');
  }
...
if (require.main === module) {
  importInstructors().catch(console.error);
}

module.exports = { importInstructors, instructorsData };
