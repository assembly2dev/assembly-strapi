 ...
 * Usage: node scripts/import-instructors-direct.js
 */
...
async function importInstructorsDirect() {
  console.log('🚀 Starting direct instructor import...');
  console.log(`📊 Prepared ${instructorsData.length} instructor profiles`);
  
  console.log('\n📝 Instructor profiles to be imported:');
  ...
  console.log('✅ Instructor data prepared successfully!');
  console.log('\n🎯 Next steps:');
  console.log('1. Start your Strapi server: npm run develop');
  console.log('2. Go to http://localhost:1337/admin');
  console.log('3. Create instructors manually using the data above');
  console.log('4. Or use the API with proper authentication');
  
  console.log('\n📋 Quick copy-paste data for manual creation:');
  console.log('=====================================');
  ...
  instructorsData.forEach((instructor, index) => {
    ...
    if (instructor.socialLinks.whatsapp) console.log(`WhatsApp: ${instructor.socialLinks.whatsapp}`);
  });
...
if (require.main === module) {
  importInstructorsDirect().catch(console.error);
}

module.exports = { importInstructorsDirect, instructorsData };
