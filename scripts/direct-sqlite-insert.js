/**
 * Direct SQLite insertion script for instructors
 * This script directly inserts instructor data into the live SQLite database
 * Usage: node scripts/direct-sqlite-insert.js
 */

const sqlite3 = require('sqlite3').verbose();
const path = require('path');

// Database file path (Strapi's SQLite database)
...
async function insertInstructorsDirectly() {
  console.log('🚀 Starting direct SQLite insertion...');
  ...
  return new Promise((resolve, reject) => {
    ...
    instructorsData.forEach((instructor, index) => {
      console.log(`\n${index + 1}. Inserting: ${instructor.name}`);
      ...
      const values = [
        ...
        instructor.showOnFacilitatorsPage ? 1 : 0,
        now,
        now,
        now
      ];
      ...
      db.run(insertQuery, values, function(err) {
        if (err) {
          console.log(`   ❌ Error inserting ${instructor.name}: ${err.message}`);
        } else {
          console.log(`   ✅ Inserted with ID: ${this.lastID}`);
          createdInstructors.push({ id: this.lastID, name: instructor.name });
        }
        
        completed++;
        if (completed === instructorsData.length) {
